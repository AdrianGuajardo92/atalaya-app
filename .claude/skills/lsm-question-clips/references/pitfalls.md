...
**Regla:** usar `detect-question-icon.swift` (umbral dinámico, exclusión 0–13,5 s).
...
bash scripts/lsm/cut-from-segments.sh scripts/lsm/segments-YYYY-MM-DD.json SOURCE --only q:1-17
```
...
1. **Validación automática:** `detect-questions.sh` debe mostrar 12 segmentos `[OK]`, 3–25 s cada uno.
2. **Frame al 50 %:** `clip-frame.swift clip.mp4 /tmp/qa.png` — debe verse `?` en bottom-right.
3. **`question-scan.csv`:** filas con `hasIcon=1` deben tener `pngBytes ≈ 4500+` y `lightRatio ≈ 0,13`.
...
| Señal | Acción |
|-------|--------|
| `expected 12 segments, got N` | Revisar CSV; recalibrar con segundo donde `?` es inequívoco |
