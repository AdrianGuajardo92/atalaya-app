...
def question_to_data(question: QuestionDraft) -> dict[str, object]:
...
def article_to_data(article: ArticleDraft) -> dict[str, object]:
    return {
        ...
        "theme": article.theme,
        "questions": [question_to_data(question) for question in article.questions],
        "paragraphs": [
            {"number": paragraph.number, "content": paragraph.content, "summary": paragraph.summary}
...
def render_article_file(article: ArticleDraft) -> str:
    ...
    return (
        ...
        "// ============================================\n"
        f"export const biblicalTexts{article.article_number}: Record<string, {{ reference: string; text: string }}[]> = {json_ts(biblical_texts)};\n\n"
        "// ============================================\n"
        f"// DATOS DEL ARTÍCULO {article.article_number}\n"
        "// ============================================\n"
        f"export const article{article.article_number}: ArticleData = {json_ts(article_to_data(article))};\n"
    )
...
def write_article_files(articles: list[ArticleDraft], articles_dir: Path, force: bool) -> list[Path]:
    ...
    for article in articles:
        path = articles_dir / f"article-{article.article_number}.ts"
...
def update_index(repo_root: Path, articles: list[ArticleDraft]) -> None:
    ...
    numbers = set(int(value) for value in re.findall(r"article(\d+)", text))
    numbers.update(article.article_number for article in articles)
    numbers = set(number for number in numbers if number > 0)
    ordered = sorted(numbers)
    imports = "\n".join(f"import {{ article{n}, biblicalTexts{n} }} from './article-{n}';" for n in ordered)
    text = re.sub(
    ...
    articles_map = "export const articlesMap: Record<number, ArticleData> = {\n" + "\n".join(f"  {n}: article{n}," for n in ordered) + "\n};"
    text = re.sub(r"export const articlesMap: Record<number, ArticleData> = \{\n.*?\n\};", articles_map, text, flags=re.S)
    bible_map = "export const biblicalTextsMap: Record<number, Record<string, { reference: string; text: string }[]>> = {\n" + "\n".join(f"  {n}: biblicalTexts{n}," for n in ordered) + "\n};"
    text = re.sub(r"export const biblicalTextsMap: Record<number, Record<string, \{ reference: string; text: string \}\[\]>> = \{\n.*?\n\};", bible_map, text, flags=re.S)
    exports_lines = []
    for n in ordered:
        suffix = "," if n != ordered[-1] else ""
        exports_lines.append(f"  article{n}, biblicalTexts{n}{suffix}")
    exports_block = "export {\n" + "\n".join(exports_lines) + "\n};"
    text = re.sub(r"export \{\n\s*article\d+.*?\n\};\s*$", exports_block, text, flags=re.S)
...
def update_config(repo_root: Path, articles: list[ArticleDraft]) -> None:
    ...
    text = path.read_text(encoding="utf-8")
    existing = set(int(value) for value in re.findall(r"^\s*(\d+),\s*//", text, flags=re.M))
    existing.update(article.article_number for article in articles)
    ...
    for number in sorted(existing):
        ...
        if article:
            ...
            lines.append(f"    {number},  // \"{title}\" ({article.week})")
        else:
            old_comment = re.search(rf"^\s*{number},\s*//\s*(.*)$", text, flags=re.M)
            comment = old_comment.group(1) if old_comment else "artículo existente"
            lines.append(f"    {number},  // {comment}")
    active_block = "activeArticles: [\n" + "\n".join(lines) + "\n  ],"
    text = re.sub(r"activeArticles: \[\n.*?\n\s*\],", active_block, text, flags=re.S)
    first_article = sorted(articles, key=lambda article: article.article_number)[0]
    ...
    text = re.sub(r"defaultArticleNumber: \d+|defaultArticleNumber: null", f"defaultArticleNumber: {first_article.article_number}", text)
    path.write_text(text, encoding="utf-8")
...
def validate_articles(articles: list[ArticleDraft]) -> list[str]:
    ...
    for article in articles:
        if not article.questions:
            warnings.append(f"Artículo {article.article_number}: no tiene preguntas.")
        if not article.paragraphs:
            warnings.append(f"Artículo {article.article_number}: no tiene párrafos.")
        for paragraph in article.paragraphs:
            plain_summary = clean_text(strip_bold(paragraph.summary))
            plain_content = clean_text(paragraph.content)
            if not plain_summary:
                warnings.append(f"Artículo {article.article_number}, párrafo {paragraph.number}: falta summary.")
            if plain_summary.startswith("¿"):
                warnings.append(f"Artículo {article.article_number}, párrafo {paragraph.number}: el summary empieza como pregunta.")
            if plain_summary.endswith("..."):
                warnings.append(f"Artículo {article.article_number}, párrafo {paragraph.number}: el summary parece truncado.")
            if len(plain_summary) > 260:
                warnings.append(f"Artículo {article.article_number}, párrafo {paragraph.number}: el summary es demasiado largo.")
            if len(plain_summary) > 120 and plain_summary in plain_content:
                warnings.append(f"Artículo {article.article_number}, párrafo {paragraph.number}: el summary parece copiado del párrafo.")
        for question in article.questions:
            if question.read_text and question.read_text not in article.biblical_texts:
                warnings.append(f"Artículo {article.article_number}: falta biblicalTexts para {question.read_text}.")
            for link in question.bible_links:
                if not link.text:
                    warnings.append(f"Artículo {article.article_number}: falta texto bíblico para {link.reference}.")
        for question in article.questions:
            if question.image and "_xl" not in question.image:
                warnings.append(f"Artículo {article.article_number}: imagen no parece ser tamaño XL en pregunta {question.number}.")
    return warnings
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, help="Directorio alterno para escribir article-XX.ts.")
    ...
    existing = existing_article_numbers(args.repo_root)
    plan: list[tuple[IssueEntry, int, bool]] = []
    for index, entry in enumerate(entries):
        number = first_number + index if first_number else article_number_from_doc_id(entry.doc_id, args.article_offset)
        plan.append((entry, number, number in existing))

    for entry, number, exists in plan:
        status = "existe" if exists else "nuevo"
        print(f"- Artículo {number} ({status}) docId {entry.doc_id}: {entry.title} [{entry.context}]")
    ...
    for entry, number, exists in plan:
        if exists and not args.force:
            continue
        print(f"Generando artículo {number}: {entry.title}")
