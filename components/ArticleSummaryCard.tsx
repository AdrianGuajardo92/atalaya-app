...
export function ArticleSummaryCard({ summary, articleTitle }: ArticleSummaryCardProps) {
  ...
  return (
    <section className="my-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 rounded-xl border border-blue-100 dark:border-blue-800 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={handleCopy}
          className="p-2 rounded-lg border border-blue-200 dark:border-blue-700 hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors"
      ...
      <div className="space-y-3">
        <p className="text-sm text-blue-700 dark:text-blue-300 font-medium">Oraciones clave del artículo:</p>
        <ol className="list-decimal list-inside space-y-2 text-text-body">
          {summary.keyPoints.map((point, idx) => (
            <li key={idx} className="leading-relaxed">
              {point.statement}
              {point.bibleReference && (
                <span className="text-blue-600 dark:text-blue-400 ml-1">({point.bibleReference})</span>
              )}
            </li>
          ))}
        </ol>
      ...
      <div className="mt-6 p-4 bg-surface/60 rounded-lg border-l-4 border-blue-400 dark:border-blue-500">
        <p className="text-sm text-blue-700 dark:text-blue-300 font-medium mb-1 flex items-center gap-1">
